...
//   ※ 원래 kimbomi-site 에 두려 했으나 Hobby 플랜 서버리스 12개 상한에 걸려 art1987(함수 2개) 로 옮김.
...
module.exports = async function (req, res) {
  ...
  var okOrigin = /^https:\/\/(www\.)?art1987\.kr$|^https:\/\/(www\.)?xn--4k0b53xuva\.com$|\.vercel\.app$|^https:\/\/kimbomi0603\.github\.io$|^https?:\/\/localhost(:\d+)?$/.test(origin.replace(/\/$/, ""));
  if (origin && okOrigin) {
    ...
    for (var k = 0; k < GROQ_MODELS.length; k++) {
      try {
        var r = await fetch("https://api.groq.com/openai/v1/chat/completions", {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: "Bearer " + GROQ, "User-Agent": "Mozilla/5.0 art1987" },
